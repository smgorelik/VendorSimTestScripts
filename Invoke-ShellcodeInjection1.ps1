<#
.SYNOPSIS
Shellcode injection and execution is one of the most popular living-off-the-land techniques, 
the motivation of this technique is to execute a small malicious and evasive code that usually will 
remotely load next stage backdoor

.DESCRIPTION
This injection will load WinExec metasploit basic calculator innovation shellcode into a legitimate notepad.exe process.
If successful it will simulate execution of a backdoor spawned by windows notepad.exe legitimate process
...
function Invoke-Shellcode1 {
    ...
    Begin {
		...
		Add-Type $ShellcodeCsharp -CompilerParameters $cp
    }

    Process {
		[Shellcode.Loader]::Run()
    }
